...
class SetsMapEntry<E, V> {
  ...
  factory SetsMapEntry.withIterableK(Iterable<E> keys, V value) => SetsMapEntry(keys.toSet(), value);
...
class SetsMap<E, V> {
  ...
  V? operator [](Object? key) {
    ...
    if (key is List<E>) {
      for (var entry in entries) {
        if (key
            .toSet()
            .intersection(entry.keys)
            .isNotEmpty) return entry.value;
      }
    }
    if (key is Set<E>) {
      for (var entry in entries) {
  ...
  V? remove(Object? key) {
    ...
    Set<E>? keys = (key is List<E>) ? key.toSet() : (key is Set<E>
        ? key
        : null);
    if (keys != null) {
      for (int i = entries.length - 1; i >= 0; i--) {
        if (keys
            .intersection(entries[i].keys)
            .isNotEmpty) {
          var val = entries[i].value;
          entries.remove(i);
          return val;
  ...
  void addAll<K>(Iterable<K> other) {
    if (K is SetsMapEntry<E,V>) {
      entries.addAll(other as Iterable<SetsMapEntry<E, V>>);
    }
    if (K is MapEntry<Set<E>,V>) {
      entries.addAll((other as Iterable<MapEntry<Set<E>,V>>).map((e) => SetsMapEntry(e.key, e.value)));
    }
    if (K is (Set<E>,V)) {
      entries.addAll((other as Iterable<(Set<E>,V)>).map((e) => SetsMapEntry(e.$1, e.$2)));
    }
  }
